...
aadAppId=$(az ad app list --display-name geogpt_web_access_$RANDOM_STRING --output tsv --query [].appId)
if [ -z $aadAppId ]; then
    echo "Creating new AD App Registration: geogpt_web_access_$RANDOM_STRING"   
    aadAppId=$(az ad app create --display-name geogpt_web_access_$RANDOM_STRING --sign-in-audience AzureADMyOrg --identifier-uris "api://geogpt-$RANDOM_STRING" --web-redirect-uris "https://geogpt-web-$RANDOM_STRING.azurewebsites.net/.auth/login/aad/callback" --enable-access-token-issuance true --enable-id-token-issuance true --output tsv --query "[].appId")
    aadAppId=$(az ad app list --display-name geogpt_web_access_$RANDOM_STRING --output tsv --query [].appId)
fi

aadSPId=$(az ad sp list --display-name geogpt_web_access_$RANDOM_STRING --output tsv --query "[].id")
if [ -z $aadSPId ]; then
    echo "Creating new AD Service Principal: geogpt_web_access_$RANDOM_STRING"
