...
if [ -n "${IN_AUTOMATION}" ]; then
  ...
  aadAppId=$(az ad app list --display-name geowise_web_access_$RANDOM_STRING --output tsv --query [].appId)
  if [ -z $aadAppId ]
    then
      aadAppId=$(az ad app create --display-name geowise_web_access_$RANDOM_STRING --sign-in-audience AzureADMyOrg --identifier-uris "api://geowise-$RANDOM_STRING" --web-redirect-uris "https://geowise-web-$RANDOM_STRING.azurewebsites.net/.auth/login/aad/callback" --enable-access-token-issuance true --enable-id-token-issuance true --output tsv --query "[].appId")
      aadAppId=$(az ad app list --display-name geowise_web_access_$RANDOM_STRING --output tsv --query [].appId)
    fi
  
  aadSPId=$(az ad sp list --display-name geowise_web_access_$RANDOM_STRING --output tsv --query "[].id")
  if [ -z $aadSPId ]; then
